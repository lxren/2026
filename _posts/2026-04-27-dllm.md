...
These problems can be resolved if the model can dynamically adapt its output length for each query. To this end, Li et al. <d-cite key="li2025beyondfixed"></d-cite>introduce the inventively named DAEDAL (**D**ynamic **A**daptive Length **E**xpansion for **D**iffusion L**a**rge **L**anguage Models), a training-free decoding technique that leverages `<EOS>` token probabilities to dynamically adjust output length. 
